...
6. **データ収集と保存**: 参加者の反応と音楽経験アンケートを記録し、CSVファイルとして保存するメカニズム
...
- **`templates/`**: 異なる実験段階のHTMLテンプレートを含むディレクトリ：
  ...
  - **`questionnaire.html`**: 音楽経験アンケートページ。
...
5. **アンケート機能**:
   - `save_questionnaire_details()`: 音楽経験アンケートの詳細回答を個別CSVファイルに保存
   - `update_summary_with_questionnaire()`: アンケート合計スコアをサマリーファイルに統合
...
5. **アンケート回答**: 実験完了後、音楽経験に関する7問のアンケートに回答
6. **結果表示**: 全ブロック完了後、レベル判定付きの結果と心理測定曲線を表示
...
実験データは以下のディレクトリ構造で保存されます：

```
data/
└── YYYY-MM-DD/
    └── participant_id/
        ├── participant_id_summary.csv          # 実験結果のサマリー（MLE分析結果含む）
        ├── participant_id_experiment_*.csv     # 各条件の詳細な試行データ
        └── participant_id_questionnaire_details.csv  # アンケート詳細回答（NEW!）
```
...
- **timestamp**: アンケート送信時刻
- **q1～q7**: 各質問の個別回答（1-5点）
- **total_score**: 合計スコア（7-35点）

この詳細データにより、音楽経験と実験パフォーマンスの相関分析が可能になります。
